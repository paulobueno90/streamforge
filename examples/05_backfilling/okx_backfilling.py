"""
OKX Backfilling - Load Historical Data
=======================================

Backfill historical candle data from OKX.

Uses OKX's public API to fetch historical data (no API keys required).

Prerequisites:
- streamforge installed

Run:
    python okx_backfilling.py
"""
...
def example1_backfill_to_csv():
    ...
    backfiller = sf.OkxBackfilling(
        symbol="BTC-USDT",  # OKX uses hyphenated format
        timeframe="1m",
    ...
    print("  Output: Okx-BTC-USDT-1m-2025-10-01_2025-10-07.csv")
...
def example7_backfill_different_timeframes():
    """
    Example 7: Backfill Different Timeframes
    
    Load data at different intervals (1m, 5m, 15m, 1h, 4h, 1d)
    """
    print("\n" + "="*60)
    print("Example 7: Backfill Multiple Timeframes")
    print("="*60)
    ...
    for timeframe in timeframes:
        ...
        print(f"✓ {timeframe} complete!")
    
    print("\n✓ All timeframes backfilled!")


def main():
    """
    OKX Backfilling Tips:
    
    ✓ OKX uses hyphenated symbol format: BTC-USDT, ETH-USDT
...
7. example7_backfill_different_timeframes() - Multiple timeframes
...
""")
    ...
    # example7_backfill_different_timeframes()
